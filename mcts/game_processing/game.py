...
from mcts.mcts_algorithm import State, Tree
...
def mcts_two_player_step(env, n_iterations, T_max=50, use_uct=False, is_pursuer=False):
    ...
    visited_states = {make_str_state(x_e=x_e, x_p=x_p, is_pursuer=is_pursuer): True}
    ...
    for k in range(n_iterations):
        ...
        for a_e, action_e in enumerate(a):  # iterate over all actions of evader
            for a_p, action_p in enumerate(a):  # iterate over all actions of pursuer
                ...
                if make_str_state(x_e=new_x_e, x_p=new_x_p, is_pursuer=is_pursuer) not in visited_states.keys():
                    ...
                    visited_states[make_str_state(x_e=new_x_e, x_p=new_x_p, is_pursuer=is_pursuer)] = True
                else:
                    continue
                ...
                tree.update_tree(env=env, node_id=-1, outcome=sum_distance)
    ...
    if is_pursuer:
        u = tree.states[u[0][0]].action_applied_p
    else:
